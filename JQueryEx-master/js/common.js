...
$(function(win){
	...
	//문서의 화면 크기 (계속 변하는 값이라 var로 변수지정해줌)
	...
	$(function(){
					...
					function gnbPlay() {
						...
						//모든 보이는 서브메뉴를 닫는다
							$("#gnb ul ul:visible").slideUp(300);
							//현재 이벤트로 선택된 a태그(메뉴)의 형제(ul서브메뉴)를 보여준다
							$ts.next().stop(true,true).slideDown(300);
						}
					} 
					...
					function gnbleave() {
						/* 직접 작성할 공간 */
						$("#gnb ul ul:visible").slideUp(300); // 보이는 전체 세버메뉴를 닫고
						$("#gnb>ul>li>a").removeClass("on"); // 메뉴부분의 on 클래스를 지워준다.
					}
